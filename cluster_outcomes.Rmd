...
renv::install(c("survival", "survminer", "cmprsk", "MASS"))
...
oneyear_analysisdata$cluster_pam <- factor(
  oneyear_analysisdata$cluster_pam,
  labels = paste("Cluster", 1:length(unique(oneyear_analysisdata$cluster_pam)))
)
...
# 4. Fit KM model
km_fit_pam <- survfit(surv_objPAM ~ cluster_pam, data = oneyear_analysisdata)
...
km_plot_pam <- ggsurvplot(
  ...
  legend.labs = levels(oneyear_analysisdata$cluster_pam),
  risk.table = TRUE,
...
cuminc_death <- cuminc(ftime   = oneyear_analysisdata$ftime, fstatus = oneyear_analysisdata$fstatus, group   = oneyear_analysisdata$cluster_pam)
print(cuminc_death)
...
# 4. Design matrices for Models 1–5 (same as before)
