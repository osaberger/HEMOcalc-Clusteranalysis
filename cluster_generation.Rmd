...
```
>>>>>>> ba1cef3 (Update cluster_outcomes and workflow files)
...
clustering_variables <- oneyear_analysisdata %>%
  ...
  ungroup() %>%
  dplyr::select(noxpthdil_v0, beta_crosslaps_v0, opg_v0,
         lmanox_v0, perox_v0, srankl_v0, crp_v0) %>%
  as.data.frame()
...
```{r radar-charts, fig.width=7, fig.height=5}
# 1. Variables, colors and labels used for radar charts
radarchart_vars <- c("noxpthdil_v0", "beta_crosslaps_v0",
...
# Reproducibility: include session info at the end of the doc
sessionInfo()
```